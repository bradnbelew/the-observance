...
/** Primary-thread Paper world port for the bounded M12 installer. */
public final class BukkitBranchGovernanceWorld implements BranchGovernanceInstaller.WorldPort {
    private final World world; private final Origin origin; private final String binding;
    public BukkitBranchGovernanceWorld(World world, Origin origin) {
    ...
    @Override public boolean matches(Cell relative, String expectedBlockData) {
        requirePrimaryThread(); BlockData expected = Bukkit.createBlockData(expectedBlockData); return block(relative).getBlockData().matches(expected); }
    @Override public boolean isAir(Cell relative) { requirePrimaryThread(); return block(relative).getType().isAir(); }
    @Override public void setBlockData(Cell relative, String blockData) { requirePrimaryThread(); block(relative).setBlockData(Bukkit.createBlockData(blockData), false); }
    private Block block(Cell relative) { return world.getBlockAt(origin.x() + relative.x(), origin.y() + relative.y(), origin.z() + relative.z()); }
    private static void requirePrimaryThread() { if (!Bukkit.isPrimaryThread()) throw new IllegalStateException("M12 world access requires Paper's primary thread"); }
}
