...
import com.observance.watcher.morrow.witnessanchor.BukkitWitnessAnchor;
import com.observance.watcher.morrow.witnessanchor.BukkitWitnessAnchorWorld;
import com.observance.watcher.morrow.witnessanchor.WitnessAnchorInstaller;
import com.observance.watcher.morrow.witnessanchor.WitnessAnchorManifest;
import com.observance.watcher.morrow.almost.AlmostHomeInstaller;
import com.observance.watcher.morrow.almost.AlmostHomeManifest;
import com.observance.watcher.morrow.almost.BukkitAlmostHome;
import com.observance.watcher.morrow.almost.BukkitAlmostHomeWorld;
import com.observance.watcher.morrow.continuity.AccountContinuityInstaller;
...
public final class MorrowRuntime implements AutoCloseable {
    ...
    public static MorrowRuntime start(ObservancePlugin plugin) throws IOException {
        ...
        WitnessAnchorInstaller.Result witnessAnchorResult = null;
        WitnessAnchorInstaller.Origin witnessAnchorOrigin = null;
        if (section.getBoolean("witness-anchor.build-enabled", false)) {
            if (consensusAuditResult == null) {
                throw new IllegalArgumentException("M07 witness anchor requires the M06 consensus audit");
            }
            witnessAnchorOrigin = new WitnessAnchorInstaller.Origin(
                    section.getInt("witness-anchor.origin-x", 96),
                    section.getInt("witness-anchor.origin-y", 80),
                    section.getInt("witness-anchor.origin-z"));
            WitnessAnchorManifest.Bounds bounds = WitnessAnchorManifest.BOUNDS;
            if (witnessAnchorOrigin.y() + bounds.minimumY() < world.getMinHeight()
                    || witnessAnchorOrigin.y() + bounds.maximumY() >= world.getMaxHeight()) {
            ...
            witnessAnchorResult = new WitnessAnchorInstaller(
                    ...
                    new BukkitWitnessAnchorWorld(world, witnessAnchorOrigin));
        }
        AlmostHomeInstaller.Result almostHomeResult = null;
        AlmostHomeInstaller.Origin almostHomeOrigin = null;
        if (section.getBoolean("almost-home.build-enabled", false)) {
            if (witnessAnchorResult == null) {
                throw new IllegalArgumentException("M08 Almost Home requires the M07 witness anchor");
            }
            almostHomeOrigin = new AlmostHomeInstaller.Origin(
                    section.getInt("almost-home.origin-x", 128),
                    section.getInt("almost-home.origin-y", 80),
                    section.getInt("almost-home.origin-z"));
            AlmostHomeManifest.Bounds bounds = AlmostHomeManifest.BOUNDS;
            if (almostHomeOrigin.y() + bounds.minimumY() < world.getMinHeight()
                    || almostHomeOrigin.y() + bounds.maximumY() >= world.getMaxHeight()) {
            ...
            almostHomeResult = new AlmostHomeInstaller(
                    ...
                    new BukkitAlmostHomeWorld(world, almostHomeOrigin));
        }
        AccountContinuityInstaller.Result accountContinuityResult = null;
        AccountContinuityInstaller.Origin accountContinuityOrigin = null;
        if (section.getBoolean("account-continuity.build-enabled", false)) {
            if (almostHomeResult == null) {
                throw new IllegalArgumentException("M09 Account Continuity requires M08 Almost Home");
            }
            accountContinuityOrigin = new AccountContinuityInstaller.Origin(
                    section.getInt("account-continuity.origin-x", 160),
                    section.getInt("account-continuity.origin-y", 80),
                    section.getInt("account-continuity.origin-z"));
            AccountContinuityManifest.Bounds bounds = AccountContinuityManifest.BOUNDS;
            if (accountContinuityOrigin.y() + bounds.minimumY() < world.getMinHeight()
                    || accountContinuityOrigin.y() + bounds.maximumY() >= world.getMaxHeight()) {
        ...
        try {
            ...
            if (witnessAnchorResult != null && witnessAnchorOrigin != null && consensusAudit != null) {
                witnessAnchor = new BukkitWitnessAnchor(
                        plugin, world, witnessAnchorOrigin, settings.releaseId(), state, data);
                witnessAnchor.start();
                plugin.getLogger().info("MORROW_WITNESS_ANCHOR_READY status=" + witnessAnchorResult.status()
                        + " manifest=" + witnessAnchorResult.manifestSha256()
                        + " blocks=" + witnessAnchorResult.blockCount());
            }
            if (almostHomeResult != null && almostHomeOrigin != null && witnessAnchor != null) {
                almostHome = new BukkitAlmostHome(
                        plugin, world, almostHomeOrigin, settings.releaseId(), state, data);
                almostHome.start();
                plugin.getLogger().info("MORROW_ALMOST_HOME_READY status=" + almostHomeResult.status()
                        + " manifest=" + almostHomeResult.manifestSha256()
                        + " blocks=" + almostHomeResult.blockCount());
            }
            if (accountContinuityResult != null && accountContinuityOrigin != null && almostHome != null) {
                accountContinuity = new BukkitAccountContinuity(
                        plugin, world, accountContinuityOrigin, settings.releaseId(), state, data);
                accountContinuity.start();
                plugin.getLogger().info("MORROW_ACCOUNT_CONTINUITY_READY status=" + accountContinuityResult.status()
    ...
    public Optional<WitnessAnchorInstaller.Result> witnessAnchorResult() {
        return Optional.ofNullable(witnessAnchorResult);
    }

    public Optional<AlmostHomeInstaller.Result> almostHomeResult() {
        return Optional.ofNullable(almostHomeResult);
    }

    public Optional<AccountContinuityInstaller.Result> accountContinuityResult() {
