...
-- CONTRACT (P0-D2): the plugin upsert conflict-targets the TEXT column `owner_uuid` (one base per
-- keeper), NOT the bigint `id`. bases.id stays the bigint PK (untouched). Idempotent adds + a
-- named unique index the `on_conflict=owner_uuid` upsert resolves against.
