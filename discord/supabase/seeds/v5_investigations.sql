...
-- The exact-match oracle is limited to short decoded artifacts and explicit compatibility/recovery
-- tokens. Interpretive primary paths use the semantic web/Discord predicates or physical Paper
-- actions. No compatibility row requires a player to guess and restate a hidden prose sentence.
...
values
  ('v5-lc05-motive','C02 / founding motive recovery token',array['heat water cover','heat water shelter'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_lc05_motive":true},"node_key":"LC05"}'::jsonb,1,true,8,'phrase','{"v5_lc04_safety_script":true}'::jsonb),
  ('v5-ko03-crack-map','C03 / Orin certificate recovery token',array['conditional load brace','conditional brace failure'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_ko03_affidavit":true},"node_key":"KO03"}'::jsonb,2,true,8,'phrase','{"v5_ko02_dials":true}'::jsonb),
  ...
  ('v5-kb03-altered-watch','C03 / Brann timestamp recovery token',array['toma bell eight','toma eight nessa later'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_kb03_affidavit":true},"node_key":"KB03"}'::jsonb,2,true,8,'phrase','{"v5_kb02_toll":true}'::jsonb),
('v5-ki03-iss-correction','C03 / Iss correction recovery token',array['registrar signed out','iss signed registrar out'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_ki03_affidavit":true},"node_key":"KI03"}'::jsonb,2,true,8,'phrase','{"v5_ki02_keepsake":true}'::jsonb),
  ('v5-cw05-counterfeit','C04 / counterfeit invoice recovery token',array['false filters','counterfeit filters'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_cw05_counterfeit":true},"node_key":"CW05"}'::jsonb,2,true,8,'phrase','{"v5_cw04_roster":true}'::jsonb),
  ...
  ('v5-cw08-clear-nessa','C04 / Nessa correction recovery action',array['clear nessa','nessa cleared'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_case_c04_complete":true},"node_key":"CW08"}'::jsonb,2,true,8,'phrase','{"v5_cw07_cache":true}'::jsonb),
  ('v5-bi08-break-inquest','C05 / Break synthesis recovery token',array['fracture delay cut feedback','multi cause break'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_case_c05_complete":true},"node_key":"BI08"}'::jsonb,3,true,8,'phrase','{"v5_bi01_lamp":true,"v5_bi02_cairn":true,"v5_bi03_coop":true,"v5_bi04_well":true,"v5_bi05_watch":true,"v5_bi06_warm":true,"v5_bi07_threshold":true}'::jsonb),
  ('v5-a01-camp-ash','C07 / Camp Ash bearing recovery token',array['map supply not distance','wren distance false'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_a01_location":true},"node_key":"A01"}'::jsonb,4,true,8,'phrase','{"v5_case_c06_complete":true}'::jsonb),
...
('v5-a10-private-window','C07 / private revision recovery token',array['inside access sender unresolved','private before public'],'main_beat','{"voice_key":"oracleMainBeat","set_flags":{"v5_case_c07_complete":true},"node_key":"A10"}'::jsonb,4,true,8,'phrase','{"v5_a09_spool":true}'::jsonb),
...
values
  ...
  ('A10','C07',10,'Preserve the private revision window','prior_case','cross-source deduction','provenance conclusion',array['v5_a09_spool'],'v5_case_c07_complete','Witness Spool and open sender question','tiered version-chain hint','v5-a10-private-window'),
...
values
  ...
  ('v5-ki03-iss-correction',3,'The actor is ISS; action SIGNED OUT; subject is the unnamed registrar.',true),
  ...
  ('v5-a10-private-window',1,'Put the private counter-mark, spool intake, and public upload on one clock.',true),
  ('v5-a10-private-window',2,'The private mark is 10:19, the spool intake is 10:23, and the first public upload is 11:02.',true),
  ('v5-a10-private-window',3,'The chain proves access inside the four-person company. It does not yet identify the sender.',true),
