...
| M02 | Six-cell Static Restore proposal, native classification, unsupported-block proof, rollback baseline, curious-state transition | Authority tests; exact Paper Room 04 build/restart; bounded terminal-dialog visual checkpoint | Full non-op correct/wrong/Escape visual interaction |
| M03 | Thirty-seven-second display replay, missing player role, pulse/inventory timing, captioned earned voice fragment | Authority tests; exact Paper loop/restart; OGG hash and transcript audit | Continuous client observation with audio-on/off cue parity |
| M04 | Consented bounded movement recording, local-only samples, sealed derivative, later behavior match, private contradiction flow | 1/2/6 authority/restart/outage tests; local diagram; historical Gateway message transport | Human movement experiment plus database-bound linked-player ephemeral flow |
...
| M06 | Six immutable private vote receipts, public denominator, dissent marking, incomplete-consensus classification | Paper-proven: 1,309 blocks, 8 displays, stable restart | Six-player private receipt distribution and native classification playthrough |
...
| --- | --- | --- | --- |
| Minecraft | M01 handoff through M12 persistent coda, local-first journal, bounded worlds, native dialogs, body/replays, restart recovery | All twelve authorities; exact Paper runtime checkpoints; safe real-client cohort joins/restarts for 1/2/6 | Complete operator-free graphical playthrough and every interaction/parity lane |
