...
- that lifecycle exposed a real restart defect: unwaxed authored copper naturally advanced from
  ...
  receipt-bound forward natural aging before audit, and cancels only forward unwaxed oxidation inside
  the nine enabled Morrow structure bounds. Waxed, foreign, reverse-stage, and out-of-bounds changes
  remain fail-closed. An accelerated `random_tick_speed=1000` hold followed by restoration to `3` and
  a clean restart retained all structures with Room 04 reporting zero repairs; evidence is bound at
  `morrow/rehearsal/copper-stability/latest.json`
