...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| hold-address-reconstruction | Hold Copy | required | partial | `the-hold.zip` reached through Copperline service `1842` | Four ordered host fragments, a common-web ending, and five service digits distributed across the six-room walk | The dead listing was only the road to the copy; the live address survived inside it | Assemble `snoikerz.com:25569`, gather the group there, then say `kept` in `#the-record` | place + item + route + memory | Successful join and the Record ignition confirm it | `dashboard/public/the-hold/the-hold.zip`; `tools/build_hold_prologue.py`; `tools/check_hold_invitation.ps1`; `discord/src/oracle/web-audit.ts` | ready |
