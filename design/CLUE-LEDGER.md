...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| hold-address-reconstruction | Hold Copy | required | partial | `the-hold.zip` reached through Copperline service `1842` | Four ordered host fragments, a common-web ending, and five service digits distributed across the six-room walk | The dead listing was only the road to the copy; the live address survived inside it | Assemble `snoikerz.com:25569`, gather the group there, then find the retired Copperline relay near first arrival | place + item + route + memory | Successful join and the relay room confirm it | `dashboard/public/the-hold/the-hold.zip`; `tools/build_hold_prologue.py`; `tools/check_hold_invitation.ps1` | ready |
| discord-callback-relay | Live Surface | required | true | `discord_relay` plus Copperline ticket `1842` | Four scrambled line cards paired with bright, exposed, weathered, and oxidized copper jackets | The old service retained a remote room whose destination can be recovered without embedding an expiring invite | Sort oldest-to-newest, submit callback `9137`, enter Discord, run `/link ExactMinecraftUsername`, then write `kept` once in `#the-record` when all are bound | record + material-age ordering + web form + identity binding | The Watcher's one-shot ignition receipt confirms the cross-surface handoff | `StructureTemplates.discordRelay`; `/obs placerelay`; `dashboard/src/lib/discord-relay.ts`; `discord/src/bot/*` | ready |
