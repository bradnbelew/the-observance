...
Launch placement can still use `site plan`, `site set`, and `placeworld`, but the project must remain testable without surveying the whole world. The rehearsal lab is the proof surface. For the final production geography, first run `tools\new_launch_placement_packet.ps1`; it creates `00-placement.md`, `launch-sites.csv`, and `coords-capture.csv` so the 67-site survey pass has an external worksheet for coordinates, placement lane, intent, placeworld receipts, proof shots, cohesion notes, and visual verdicts. In game, start with `/obs site plan lanes`, then work one lane at a time with `/obs site next prologue|keepers|customs|human|deep|dread|dimensions` instead of treating all sites as one giant checklist.
Run `tools\check_launch_coord_quality.ps1 -CaptureCsv <packet>\coords-capture.csv` before treating the worksheet as real. It catches duplicate anchors, missing `/obs placeworld` receipts, wrong Nether/End dimensions, cramped keeper evidence sites, over-spread route clusters, missing proof shots, and non-`KEEP` launch rows. If the worksheet is the source of truth for the production pass, preview it with `tools\apply_launch_coords.ps1 -CaptureCsv <packet>\coords-capture.csv` before writing. The apply tool refuses open visual verdicts by default; use `-Apply` only after each row is real, placed, proofed, stamped where needed, and `KEEP`.
...
`placehold` is the production shortcut for the underground Deep Hold: it carves the protected stair, Return Mouth, Keeper Court, archive/market wings, Lamp-works descent, threshold court, and side-hush path as one controlled underground city. `prepworld` remains a rehearsal-only platform board around the operator. It places the prologue, surface spine, deep payoff sites, Lamp-works/Stair dialogue proof chain, school-stand proof, far-water mirror, marker-row count proof, Cistern 7, watch-floor, Deep Market, ration table, third-bay breach, warm-town false-lead collapse, Mara page-lock lecterns, finale/readings, NPC row, and tester tools for fast proofing. The default spacing is 36 blocks, with manual spacing clamped to 34-48 so adjacent cells do not overlap. Facing east, rows run in parallel by phase instead of scattering across the terrain. Each compact cell is cleared before stamping so hills, grass, and stone do not fill intentional air inside the structures. Normal storage in these rehearsal structures is seeded with local ledgers, receipts, witness notes, and route records; decorative chiseled bookshelves are visibly occupied, while mechanic-owned shelves are not randomly pre-filled. Use manual `site set` to curate anchors outside the Hold, optional Nether/End lanes, or bespoke surface context. Beacon beams are retired entirely; findability must come from authored evidence, route logic, NPC/book/media hints, and placement proof rather than sky waypoints.
...
Aro's longer warm-town lie is not part of the compact descent chain, but `placehold` now clusters Deep Market plus the collapse into the production Hold, while `prepworld` and `sidepass` stage them for rehearsal. The same proof family also covers `school_stand`, `the_far_water`, `markers_row`, `cistern_7`, `watch_floor`, `set_apart_shelf`, `undercroft_seal`, `forgotten_mouth`, the ration table, and third-bay breach so the schoolroom human thread, Sella's mirror/count evidence, the bow/count proof, the light-fouling proof, dark-hours proof, entry-five proof, Orin's seal, the way-up rumor, and Deep Line taboo have physical anchors. For production, run `/obs placehold build`, `/obs placehold audit`, and `/obs placehold sync`; then survey only the remaining surface, Nether, End, Unlit, media, and bespoke context anchors with `site set`/`placeworld`.
