...
- [ ] **Normalizer-parity build test** (nothing-breaks #2 residual + nice-to-have). Still worth adding: an **executable test** that runs every seeded `accepted_answers` + each coord puzzle's canonical decoded form through BOTH normalizers (TS `normalizeAnswer` + Java `AnswerNormalizer`, plus the Greek/Turkish-İ/ligature/fullwidth battery from ORACLE.md §2) and asserts set-membership — gate any row to `active=true` only if it passes. Makes the authoring checklist executable and guards against silent cross-language normalizer drift.
