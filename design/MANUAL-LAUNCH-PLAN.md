...
- `launch-attestations.md` completed from the real Paper/client pass, including the current Supabase SQL SHA1,
  plugin jar SHA1, and hosted resource-pack SHA1.
...
| 4 | Place the 67 launch-required site coordinates | Authoring placeholders are safe no-ops, but launch needs real walkable locations with sightlines and body language. | Live world `plugins/Observance/sites.yml`; proof packet `coords-capture.csv`. | In game, use `/observance site todo`, then `/observance site plan lanes`. Work one lane at a time with `/observance site next prologue|keepers|customs|human|deep|dread|dimensions`, or inspect a specific row with `/observance site plan <siteId>`, then stand at the real anchor and run `/observance site set <siteId>`. Export/fill the lane-aware capture CSV and validate it. | After the world route exists, before rehearsal. | Every site must be discoverable from the clue or surrounding world; no floating marker, test pad, or labeled route may be required. | `check_world_build_readiness.ps1 -Launch` passes; `check_launch_coord_quality.ps1 -Launch -CaptureCsv <packet>\coords-capture.csv` passes. |
...
- 67 launch-required coordinates placed and proofed: PASS/FAIL
