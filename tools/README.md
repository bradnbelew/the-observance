...
| `run_morrow_resource_pack_rehearsal.py` / `test_morrow_resource_pack_rehearsal.py` / `check_morrow_resource_pack_receipts.py` | bounded loopback real-client `LOADED`/`DECLINED` pack handshake with exact byte and cleanup receipts; human visual parity remains separate |
