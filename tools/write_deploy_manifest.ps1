...
$manifest = [ordered]@{
  ...
  deploymentInputs = [ordered]@{
    ...
    railwayWorkerConfig = File-Entry $repoFull (Join-Path $repoFull "discord\railway.worker.json")
    railwayCronConfig = File-Entry $repoFull (Join-Path $repoFull "discord\railway.cron.json")
