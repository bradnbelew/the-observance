...
def main() -> None:
    ...
    require(data["result"] == "offline_and_disposable_private_gates_passed_external_authenticated_integrations_blocked",
    ...
    for label, reason in data["external_private_target_blockers"].items():
        require(reason and ("no " in reason.lower() or "closed" in reason.lower()
                            or "absent" in reason.lower()), f"external blocker missing: {label}")
    ...
    print("Private launch routed receipt PASS: offline/disposable gates exact; external integrations blocked")
