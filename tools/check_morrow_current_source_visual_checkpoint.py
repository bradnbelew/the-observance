...
require(runtime.get("plugin_sha256") == "65406d67787b764c25f5cf727dc710de67872fdcad749b1141736a105dc75343", "plugin hash")
...
require(client.get("source_commit") == checkpoint["source_commit"], "client source commit")
...
require("Connecting to 127.0.0.1, 25593" in client_log, "client connection evidence")
require("MorrowWitness2 joined the game" in client_log, "client join evidence")
require("MORROW_RUNTIME_READY" in server_log, "runtime-ready evidence")
require("MORROW_PLAYER_SAFE_ENTRY player=MorrowWitness2" in server_log, "safe-entry evidence")
...
require(paper.get("status") == "pass" and paper.get("source_commit") == checkpoint["source_commit"], "Paper receipt status/source")
...
require(findings.get("giant_overlay_regression_absent_in_frame") is True, "overlay finding")
