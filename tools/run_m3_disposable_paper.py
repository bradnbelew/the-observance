#!/usr/bin/env python3
"""Create and audit one new, local-only disposable Paper target for the M3 slice.

The harness is intentionally creation-only: it refuses an existing target directory and never
deletes, resets, or reuses a world. Paper and plugin jars must be supplied explicitly and are hashed
before launch. The review plugin independently requires the marker written here before it will mutate
the world.
"""
...
def configure(target: Path, paper: Path, plugin: Path, target_id: str, commit: str) -> None:
    if target.exists():
    ...
    write_text(target / "server.properties", "\n".join([
        ...
        "enable-command-block=false", "enable-rcon=false", "enable-status=false", "enforce-whitelist=true",
        "force-gamemode=true", "gamemode=adventure", "generate-structures=false", "hardcore=false",
        "level-name=m3_private_slice", "max-players=4", "motd=PRIVATE M3 DISPOSABLE REVIEW",
        "online-mode=false", "prevent-proxy-connections=true", "server-ip=127.0.0.1", "server-port=25579",
        "spawn-animals=false", "spawn-monsters=false", "spawn-npcs=false", "view-distance=4",
        "simulation-distance=3", "white-list=true", "",]))
...
def exercise(target: Path, java: str) -> dict[str, str]:
    ...
    return {"platform_confirmation": confirmation, "status": status, "build": build,
            "closed_audit": closed, "open_audit": opened, "replay": replay,
...
def package_world(target: Path) -> tuple[str, str, Path]:
    ...
    package = target / "m3-private-slice-world.zip"
...
def main() -> None:
    ...
    if sha256(paper) != PAPER_EXPECTED_SHA256:
        raise ValueError("Paper JAR does not match the platform-confirmed stable build")
    configure(target, paper, plugin, args.target_id, args.commit)
    evidence = exercise(target, args.java)
    world_hash, package_hash, package = package_world(target)
    receipt = {
        "schema_version": "1.0.0-m3-paper-receipt",
        ...
        "client_receipts": {
            ...
            "reason": "No authenticated Minecraft clients were available to this local harness."
        },
